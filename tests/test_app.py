...
def test_put_route():
    response = app.test_client().put('/hello/john', json={
        "dateOfBirth": "1982-01-20"
    })
    assert response.status_code == 204

def test_put_route_invalid_username():
    response = app.test_client().put('/hello/1234', json={
        "dateOfBirth": "1982-01-20"
    })
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Invalid username. It must contain only letters.'
