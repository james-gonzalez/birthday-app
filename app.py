...
c.execute('''CREATE TABLE IF NOT EXISTS hello (username TEXT PRIMARY KEY, dateOfBirth TEXT)''')
conn.commit()
...
def put_user(username, dateOfBirth):
    ...
    if row:
        # update existing row
        c.execute('''UPDATE hello SET dateOfBirth = ? WHERE username = ?''', (dateOfBirth, username))
    else:
        # insert new row
